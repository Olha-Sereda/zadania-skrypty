...
export default function OrderDetailsPage() {
  ...
  useEffect(() => {
    axiosClient.get(`/orders/${orderId}`).then(({ data }) => setOrder(data)).catch(() => {});
  }, [orderId]);
  ...
  return (
    <section className="max-w-lg mx-auto space-y-6">
      ...
      <p className="text-black/60">{new Date(order.created_at).toLocaleString('pl-PL')}</p>
      <p className="font-semibold">Status: <span className="status-paid">{order.status}</span></p>
